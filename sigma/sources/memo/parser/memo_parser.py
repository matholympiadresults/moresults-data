...
def parse_year(year: int, raw_dir: Path, output_dir: Path, force: bool = False) -> Path:
    ...
    if year == 2008:
        from sigma.sources.memo.parser.memo_2008_parser import parse_2008

        return parse_2008(raw_dir, output_dir, force=force)

    if year == 2009:
        from sigma.sources.memo.parser.memo_2009_parser import parse_2009

        return parse_2009(raw_dir, output_dir, force=force)

    if year == 2010:
        from sigma.sources.memo.parser.skmo_parser import (
    ...
    if year == 2013:
        from sigma.sources.memo.parser.memo_2013_parser import parse_2013

        return parse_2013(raw_dir, output_dir, force=force)

    if year == 2014:
        from sigma.sources.memo.parser.memo_2014_parser import parse_2014

        return parse_2014(raw_dir, output_dir, force=force)

    # Standard parser for 2015+
    from sigma.sources.memo.downloader.memo_downloader import get_raw_filename
