"""PDF parser for BMO results (2025, 2022, 2021)."""
...
class PDFParser(BaseParser):
    ...
    Supported years: 2025, 2022, 2021
    ...
    def __init__(self, year: int, raw_data_dir: Path):
        if year not in PDF_SOURCES:
            raise ValueError(f"PDF parser not available for year {year}")
        super().__init__(year, raw_data_dir)

    @property
    def source_url(self) -> str:
        return PDF_SOURCES[self.year]
    ...
    def download_raw(self, force: bool = False) -> Path:
        """Download PDF file and save to raw_data_dir."""
        ...
        response = requests.get(self.source_url, timeout=60, headers=headers)
